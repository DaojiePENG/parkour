...
class Go1LeapHisCfg( A1FieldCfg ):
...
class Go1LeapHisCfgPPO( A1FieldCfgPPO ):
    ...
    class runner( A1FieldCfgPPO.runner ):
        ...
        ("_pPenV" + np.format_float_scientific(-Go1LeapHisCfg.rewards.scales.penetrate_volume, trim= "-", exp_digits= 1) if getattr(Go1LeapHisCfg.rewards.scales, "penetrate_volume", 0.) < 0. else ""),
        ("_pPenD" + np.format_float_scientific(-Go1LeapHisCfg.rewards.scales.penetrate_depth, trim= "-", exp_digits= 1) if getattr(Go1LeapHisCfg.rewards.scales, "penetrate_depth", 0.) < 0. else ""),
        ("_rAlive{:d}".format(int(Go1LeapHisCfg.rewards.scales.alive)) if Go1LeapHisCfg.rewards.scales.alive != 2 else ""),
        ("_rAngVel{:.2f}".format(Go1LeapHisCfg.rewards.scales.tracking_ang_vel) if Go1LeapHisCfg.rewards.scales.tracking_ang_vel != 0.05 else ""),
        ("_pTorqueExceedIndicate" + np.format_float_scientific(-Go1LeapHisCfg.rewards.scales.exceed_torque_limits_i, trim= "-", exp_digits= 1) if getattr(Go1LeapHisCfg.rewards.scales, "exceed_torque_limits_i", 0.) != 0. else ""),
        ("_pTorqueExceedSquare" + np.format_float_scientific(-Go1LeapHisCfg.rewards.scales.exceed_torque_limits_square, trim= "-", exp_digits= 1) if getattr(Go1LeapHisCfg.rewards.scales, "exceed_torque_limits_square", 0.) != 0. else ""),
        ("_pYaw{:.2f}".format(-Go1LeapHisCfg.rewards.scales.yaw_abs) if Go1LeapHisCfg.rewards.scales.yaw_abs != 0. else ""),
        ("_noComputerTorqueClip" if not Go1LeapHisCfg.control.computer_clip_torque else ""),
        ("_virtualTerrain" if Go1LeapHisCfg.terrain.BarrierTrack_kwargs["virtual_terrain"] else ""),
        ("_aScale{:d}{:d}{:d}".format(
                int(Go1LeapHisCfg.control.action_scale[0] * 10),
                int(Go1LeapHisCfg.control.action_scale[1] * 10),
                int(Go1LeapHisCfg.control.action_scale[2] * 10),
            ) if isinstance(Go1LeapHisCfg.control.action_scale, (tuple, list)) \
            else "_aScale{:.1f}".format(Go1LeapHisCfg.control.action_scale)
        ),
        ("_torqueClip{:.0f}".format(Go1LeapHisCfg.control.torque_limits) if not isinstance(Go1LeapHisCfg.control.torque_limits, (tuple, list)) else (
            "_tClip{:d}{:d}{:d}".format(
                int(Go1LeapHisCfg.control.torque_limits[0]),
                int(Go1LeapHisCfg.control.torque_limits[1]),
                int(Go1LeapHisCfg.control.torque_limits[2]),
            )
        )),
        ])
        resume = False
